...
try {
    ...
    $sql = "select * from lwwritefromdictationques where lwwmid = :mid limit $limit, $perpage";
    $s = $conn->prepare($sql);
    $s->bindValue(':mid', $_SESSION['testid']);
    $s->execute();
    $result = $s->fetchAll();
    $query = $conn->query("select count(*) from lwwritefromdictationques where lwwmid = '".$_SESSION['testid']."'");
    $total = $query->fetch();
...
$url = "practiceWritefromDictation.html.php?page=".$next;
...
<head>
    ...
    <title>PTE Preparation - Practice Write from Dictation</title>
...
<body>
    ...
    <div class="container-fluid">
        <div class="header">
            <p> PTE Write from Dictation Practice Questions - <?php echo $_SESSION['name'];?></p>
            <div id="cover">
        ...
        <div class="heading">You will hear a sentence. Type the sentence in the box below exactly as 
            you hear it. Write as much of the sentence as you hear it. You will hear the sentence only once.</div>
  ...
  function timer(x, elem){
    var t = setInterval(function(){
        ...
        var elemContent = document.getElementById(elem);
            ...
            if(x < 0) {
                clearInterval(t);
                document.getElementById('bar').disabled;
            }
    }, 1000);
}
...
$(document).ready(function() {
  ...
  var audiosource = "serveruploads/questions/lwwritefromdictation/<?php echo trim($row['lwwfile']); ?>";		//	Audio source
