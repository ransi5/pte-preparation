...
try {
    ...
    $s->execute();
    $result = $s->fetchAll();
    $sql1 = "select * from reorderparagraphanswers where reostudid = :studid and reotestid = :testid limit $limit, $perpage";
    $s = $conn->prepare($sql1);
...
<head>
    ...
    <title>PTE Preparation - Re-Order Paragraph Answer Key</title>
...
<div class="container-fluid sidenav">
    <div class="row">
        ...
        <div class="col-md-8 mid">
            <h2>Question type: Re-order Paragraph</h2>
            <ul class="pagination">
                <?php for ($i=1; $i <= $pages; $i++) {?>
                <li><a href="?page=<?php echo $i?>">Item <?php echo $i?></a></li>
                <?php } ?>
                <li><a style="background-color: #082f6f; color: white;" href="anskeyReading,FillBlanks.html.php?page=1">Next Item Type</a></li>
            </ul>
            <?php foreach ($result as $row) {?>
            <div class="qbox">
                <h3>Item <?php echo $row['reoqno'];?></h3>
                <p>1: <?php echo $row['reooptiona'];?></p>
                <p>2: <?php echo $row['reooptionb'];?></p>
                <p>3: <?php echo $row['reooptionc'];?></p>
                <p>4: <?php echo $row['reooptiond'];?></p>
                <p>5: <?php echo $row['reooptione'];?></p>
                <?php foreach ($answer as $how) {?>
                <div class="youranswer">
                    <h3>Your Answer</h3>
                    <p class="text"><?php echo $how['reoanswer'];?></p>
                </div>
                <?php } ?>
                <div class="modanswer">
                    <h3>Model Answer</h3>
                    <p class="text"><?php echo $row['reoanskey'];?></p>
                </div>
            </div>
            <?php } ?>
