...
try {
    ...
    $sql = "select * from lwfillblankques where lwfmid = :mid limit $limit, $perpage";
    $s = $conn->prepare($sql);
    $s->bindValue(':mid', $_SESSION['testid']);
    $s->execute();
    $result = $s->fetchAll();
    $query = $conn->query("select count(*) from lwfillblankques where lwfmid = '".$_SESSION['testid']."'");
    $total = $query->fetch();
...
$url = "practiceL&W,FillBlanks.html.php?page=".$next;
...
<head>
    ...
    <title>PTE Preparation - Practice Listening & Writing, Fill in the Blanks</title>
...
if (isset($_POST['time'])) {
    try {
        $sql = 'insert into lwfillblankanswers (lwftestid, lwfquesno, lwfstudid, lwfblankanswers) values (:test, :quesno, :studid, :answer)';
        $blank = implode(',', $_POST['blank']);
        $s = $conn->prepare($sql);
        $s->bindValue(':test', $_SESSION['testid']);
        $s->bindValue(':quesno', $_SESSION['quesno']);
        $s->bindValue(':studid', $_SESSION['id']);
        $s->bindValue(':answer', $blank);
        $s->execute();
...
<body>
    ...
    <div class="gill">
        <div class="header">
            <p> PTE Listening & Writing - Fill in the Blanks Practice Questions - <?php echo $_SESSION['name'];?></p>
            <div id="cover">
        ...
        <div class="heading">You will hear a recording. Type the missing words in each blank.</div>
...
$(document).ready(function() {
  ...
  var audiosource = "serveruploads/questions/lwfillblank/<?php echo trim($row['lwffile']); ?>";		//	Audio source
