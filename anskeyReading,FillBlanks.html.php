...
try {
    ...
    $s->execute();
    $result = $s->fetchAll();
    $sql1 = "select * from rfillblankanswers where rfistudid = :studid and rfitestid = :testid limit $limit, $perpage";
    $s = $conn->prepare($sql1);
...
<head>
    ...
    <title>PTE Preparation - Reading, Fill in the Blanks Answer Key</title>
...
<div class="container-fluid sidenav">
    <div class="row">
        ...
        <div class="col-md-8 mid">
            <h2>Question type: Reading, Fill in the Blanks</h2>
            <ul class="pagination">
                <?php for ($i=1; $i <= $pages; $i++) {?>
                <li><a href="?page=<?php echo $i?>">Item <?php echo $i?></a></li>
                <?php } ?>
                <li><a style="background-color: #082f6f; color: white;" href="anskeyR&W,FillBlanks.html.php?page=1">Next Item Type</a></li>
            </ul>
            <?php foreach ($result as $row) {?>
            <div class="qbox">
                <h3>Item <?php echo $row['rfiqno'];?></h3>
                <p><?php echo $row['rficontent'];?></p>
                <p class="text">Options: <?php echo $row['rfioptiona'];?>, <?php echo $row['rfioptionb'];?>, <?php echo $row['rfioptionc'];?>, 
                    <?php echo $row['rfioptiond'];?>, <?php echo $row['rfioptione'];?>, <?php echo $row['rfioptionf'];?>, 
                    <?php echo $row['rfioptiong'];?>, <?php echo $row['rfioptionh'];?></p>
                <?php foreach ($answer as $how) {?>
                <div class="youranswer">
                    <h3>Your Answer</h3>
                    <p class="text"><?php echo $how['rfiansa'];?>, <?php echo $how['rfiansb'];?>, 
                    <?php echo $how['rfiansc'];?>, <?php echo $how['rfiansd'];?>, <?php echo $how['rfianse'];?></p>
                </div>
                <?php } ?>
                <div class="modanswer">
                    <h3>Model Answer</h3>
                    <p class="text"><?php echo $row['rfianswera'];?>, <?php echo $row['rfianswerb'];?>, <?php echo $row['rfianswerc'];?>
                    , <?php echo $row['rfianswerd'];?>, <?php echo $row['rfianswere'];?></p>
                </div>
            </div>
            <?php } ?>
