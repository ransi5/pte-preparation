...
try {
    ...
    $s->execute();
    $result = $s->fetchAll();
    $sql1 = "select * from lwfillblankanswers where lwfstudid = :studid and lwftestid = :testid limit $limit, $perpage";
    $s = $conn->prepare($sql1);
...
<head>
    ...
    <title>PTE Preparation - Listening & Writing, Fill in the Blanks Answer Key</title>
...
<div class="container-fluid sidenav">
    <div class="row">
        ...
        <div class="col-md-8 mid">
            <h2>Question type: Listening & Writing, Fill in the Blanks</h2>
            <ul class="pagination">
                <?php for ($i=1; $i <= $pages; $i++) {?>
                <li><a href="?page=<?php echo $i?>">Item <?php echo $i?></a></li>
                <?php } ?>
                <li><a style="background-color: #082f6f; color: white;" href="anskeyHighlightCorrectSummary.html.php?page=1">Next Item Type</a></li>
            </ul>
            <?php foreach ($result as $row) {?>
            <div class="qbox">
                <h3>Item <?php echo $row['lwfqno'];?></h3>
                <audio controls>
                    <source src="serveruploads/questions/lwfillblank/<?php echo $row['lwffile']; ?>" type="audio/mpeg">
                </audio>
                <p><?php echo $row['lwfcontent'];?></p>
                <?php foreach ($answer as $how) {?>
                <div class="youranswer">
                    <h3>Your Answer</h3>
                    <p class="text"><?php echo $how['lwfblankanswers'];?></p>
                </div>
                <?php } ?>
                <div class="modanswer">
                    <h3>Model Answer</h3>
                    <p class="text"><?php echo $row['lwfanswer'];?></p>
                </div>
            </div>
            <?php } ?>
