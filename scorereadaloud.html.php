...
if (isset($_POST['check']) && $_POST['check'] == 'Check') {
    ...
    $_SESSION['moctestid'] = $_POST['moctestid'];
    $_SESSION['mocstudid'] = $_POST['mocstudid'];    
}

try {
    $sql = "select * from readaloudques where reamid = :mid";
        $s = $conn->prepare($sql);
...
try {
    $sql = "select * from readaloudanswers where reastudid = :studid and reatestid = :testid";
        $s = $conn->prepare($sql);
...
if (isset($_POST['content1'])){
    ...
    $read = $cont1;
    $speak = $pron1 + $oral1;
    $readaloud = $cont1 + $pron1 + $oral1 + $read + $speak;
    try {
        $sql = 'update mocktestscores set reacontent = reacontent + :content, reapronounce = reapronounce + :pronounce, reafluency = reafluency + :fluency, reareading = reareading + :read, reaspeaking = reaspeaking + :speak, readaloudscore = readaloudscore + :aloud where mocid = :id';
        $s = $conn->prepare($sql);
        ...
        $s->bindValue(':aloud', $readaloud);
        $s->bindValue(':id', $_SESSION['mocid']);
        $s->execute();
        header('location: scorereadaloud.html.php?x=1');
        exit();
    } catch (PDOException $e) {
...
if (isset($_POST['content2'])){
    ...
    try {
        ...
        $s->bindValue(':id', $_SESSION['mocid']);
        $s->execute(); 
        header('location: scorereadaloud.html.php?x=2');
        exit();
    } catch (PDOException $e) {
        echo '<br> error updating database: ' . $e->getMessage();
        exit();
...
if (isset($_POST['content3'])){
    ...
    try {
        ...
        $s->bindValue(':id', $_SESSION['mocid']);
        $s->execute(); 
        header('location: scorereadaloud.html.php?x=3');
        exit();
    } catch (PDOException $e) {
        echo '<br> error updating database: ' . $e->getMessage();
        exit();
...
if (isset($_POST['content4'])){
    ...
    try {
        ...
        $s->bindValue(':id', $_SESSION['mocid']);
        $s->execute();  
        header('location: scorereadaloud.html.php?x=4');
        exit();
    } catch (PDOException $e) {
        echo '<br> error updating database: ' . $e->getMessage();
        exit();
...
if (isset($_POST['content5'])){
    ...
    try {
        ...
        $s->bindValue(':id', $_SESSION['mocid']);
        $s->execute();
        header('location: scorereadaloud.html.php?x=5');
        exit();
    } catch (PDOException $e) {
        echo '<br> error updating database: ' . $e->getMessage();
        exit();
...
if (isset($_POST['content6'])){
    ...
    try {
        ...
        $s->bindValue(':id', $_SESSION['mocid']);
        $s->execute();    
        header('location: scorereadaloud.html.php?x=6');
        exit();
    } catch (PDOException $e) {
        echo '<br> error updating database: ' . $e->getMessage();
        exit();
...
try {
    $sql = 'select readaloudscore from mocktestscores where mocid = :id';
    $s = $conn->prepare($sql);
        $s->bindValue(':id', $_SESSION['mocid']);
        $s->execute();
        $col = $s->fetch();
        $score = $col['readaloudscore'];
} catch (PDOException $e) {
            echo '<br> error updating database: ' . $e->getMessage();
            exit();
}
...
<head>
    ...
    <title>PTE Preparation - Read Aloud Check</title>
            ...
            <div class="container">
                ...
                <table class="quest" style="width: 100%">
                    <tr>
                        <th>Mock Test Id</th>
                        <th>Item No</th>
                        <th>Read Aloud Text</th>
                        <th>Answer Key</th>
                    </tr>
                    <?php foreach ($result as $row) { ?>
                    <tr>
                        <td style="text-align: center"><?php echo $row['reamid']; ?></td>
                        <td style="text-align: center"><?php echo $row['reaqno']; ?></td>
                        <td><?php echo $row['reaquestion']; ?></td>
                        <td>
                            <audio controls>
                                <source src="serveruploads/answers/readaloud/<?php echo $row['reaanskey']; ?>" type="audio/mpeg">
                            </audio>
                        </td>
                    </tr>
                    <?php }?>
                ...
                <table class="quest" style="width: 100%">
                    ...
                    <tr>
                        <td style="text-align: center"><?php echo $row['reatestid']; ?></td>
                        <td style="text-align: center"><?php echo $row['reaquesno']; ?></td>
                        <td>
                            <audio controls>
                                <source src="myphpuploaders/clientuploads/readaloud/<?php echo $row['reafile']; ?>" type="audio/mpeg">
                            </audio>
                        </td>
                        <td>
                            <form action="" method="post" name="MyForm<?php echo $row['reaquesno'];?>">
                            Content: <select name="content<?php echo $row['reaquesno'];?>">
                                ...
                                Pronunciation: <select name="pronunce<?php echo $row['reaquesno'];?>">
                                    <option>Choose Marks</option>
                                    <option value="5">5</option>
                                    <option value="4">4</option>
                                    <option value="3">3</option>
                                ...
                                Oral Fluency: <select name="fluency<?php echo $row['reaquesno'];?>">
                                    <option>Choose Marks</option>
                                    <option value="5">5</option>
                                    <option value="4">4</option>
                                    <option value="3">3</option>
                                ...
                                <input type="submit" name="score" value="submit" id="score<?php echo $row['reaquesno'];?>">
                            </form>
                        </td>
                    </tr>
                    <?php }?>
                ...
                <table class="quest" style="width: 100%">
                    ...
                    <tr>
                        ...
                        <td>All words are in correct sequence and there are no ommission or inclusions.</td>
                        <td>Native-like</td>
                        <td>Native-like</td>
                    </tr> 
                    <tr>
                        <td style="text-align: center">4</td>
                        <td>Only one word is out of sequence.</td>
                        <td>Advanced</td>
                        <td>Advanced</td>
                    </tr>  
                    <tr>
                        <td style="text-align: center">3</td>
                        <td>More than one and less than three words are out of sequence.</td>
                        <td>Good</td>
                        <td>Good</td>
                    </tr>  
                    <tr>
                        <td style="text-align: center">2</td>
                        <td>More than three and less than seven words are out of sequence.</td>
                        <td>Intermediate</td>
                        <td>Intermediate</td>
                    </tr>  
                    <tr>
                        <td style="text-align: center">1</td>
                        <td>More than seven words are out of sequence.</td>
                        <td>Intrusive</td>
                        <td>Intrusive</td>
                    </tr>  
                    <tr>
                        <td style="text-align: center">0</td>
                        <td>No words are in sequence.</td>
                        <td>Non-English</td>
                        <td>Non-English</td>
                    </tr>  
                </table>  
...
$(document).ready(function() {
   $('#next').click(function(){
       window.location = 'scorerepeatsentence.html.php?x=';
   });
})
