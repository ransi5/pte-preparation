...
try {
    ...
    $sql = "select * from lhighlightincorrectwordques where lhimid = :mid limit $limit, $perpage";
    $s = $conn->prepare($sql);
    $s->bindValue(':mid', $_SESSION['testid']);
    $s->execute();
    $result = $s->fetchAll();
    $sql1 = "select * from lhighlightincorrectwordanswers where lhistudid = :studid and lhitestid = :testid limit $limit, $perpage";
    $s = $conn->prepare($sql1);
    $s->bindValue(':studid', $_SESSION['id']);
    $s->bindValue(':testid', $_SESSION['testid']);
    $s->execute();
    $answer = $s->fetchAll();
    $query = $conn->query("select count(*) from lhighlightincorrectwordques where lhimid = '".$_SESSION['testid']."'");
    $total = $query->fetch();
...
<head>
    ...
    <title>PTE Preparation - Highlight Incorrect Words Answer Key</title>
...
<div class="container-fluid sidenav">
    <div class="row">
        ...
        <div class="col-md-8 mid">
            <h2>Question type: Highlight Incorrect Word</h2>
            <ul class="pagination">
                <?php for ($i=1; $i <= $pages; $i++) {?>
                <li><a href="?page=<?php echo $i?>">Item <?php echo $i?></a></li>
                <?php } ?>
                <li><a style="background-color: #082f6f; color: white;" href="anskeyWritefromDictation.html.php?page=1">Next Item Type</a></li>
            </ul>
            <?php foreach ($result as $row) {?>
            <div class="qbox">
                <h3>Item <?php echo $row['lhiqno'];?></h3>
                <audio controls>
                    <source src="serveruploads/questions/lhighlightincorrectword/<?php echo $row['lhifile']; ?>" type="audio/mpeg">
                </audio>
                <p><?php echo $row['lhicontent'];?></p>                
                <?php foreach ($answer as $how) {?>
                <div class="youranswer">
                    <h3>Your Answer</h3>
                    <p class="text"><?php echo $how['lhianswer'];?></p>
                </div>
                <?php } ?>
                <div class="modanswer">
                    <h3>Model Answer</h3>
                    <p class="text"><?php echo $row['lhicorrectanswer'];?></p>
                </div>
            </div>
            <?php } ?>
