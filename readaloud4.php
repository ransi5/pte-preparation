...
if (isset($_POST['time'])) {
    if ($_POST['time'] < 0) {
        ...
        try {
            header('location: readaloud5.php?x=' . $_POST['time'] . '&y=' . $_POST['ftime']);       
            exit();
        } catch (PDOException $e) {
            echo '<br> error updating database: ' . $e->getMessage();
            exit();
...
<head>
    ...
    <title>PTE Practice Test - Read Aloud Question 4</title>
...
function successCallback(stream){
    setTimeout(strtrecording,prepare_time*1000+2000);    
        ...
        function upload(blob){    
            ...
            xhr('myphpuploaders/readaloud4uploader.php', formData, function (fName) {
