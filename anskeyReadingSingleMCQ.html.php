...
try {
    ...
    $s->execute();
    $result = $s->fetchAll();
    $sql1 = "select * from smcreadinganswers where smcstudid = :studid and smctestid = :testid limit $limit, $perpage";
    $s = $conn->prepare($sql1);
...
<head>
    ...
    <title>PTE Preparation - Reading MCQ, Single Answer, Answer Key</title>
...
<div class="container-fluid sidenav">
    <div class="row">
        ...
        <div class="col-md-8 mid">
            <h2>Question type: MCQ, Single Answer</h2>
            <ul class="pagination">
                <?php for ($i=1; $i <= $pages; $i++) {?>
                <li><a href="?page=<?php echo $i?>">Item <?php echo $i?></a></li>
                <?php } ?>
                <li><a style="background-color: #082f6f; color: white;" href="anskeyReadingMultipleMCQ.html.php?page=1">Next Item Type</a></li>
            </ul>
            <?php foreach ($result as $row) {?>
            <div class="qbox">
                <h3>Item <?php echo $row['smcqno'];?></h3>
                <p><?php echo $row['smccontent'];?></p>
                <p><?php echo $row['smcquestion'];?></p>
                <p>a: <?php echo $row['smcoptiona'];?></p>
                <p>b: <?php echo $row['smcoptionb'];?></p>
                <p>c: <?php echo $row['smcoptionc'];?></p>
                <p>d: <?php echo $row['smcoptiond'];?></p>
                <?php foreach ($answer as $how) {?>
                <div class="youranswer">
                    <h3>Your Answer</h3>
                    <p class="text"><?php echo $how['smcanswer'];?></p>
                </div>
                <?php } ?>
                <div class="modanswer">
                    <h3>Model Answer</h3>
                    <p class="text"><?php echo $row['smcanskey'];?></p>
                </div>
            </div>
            <?php } ?>
