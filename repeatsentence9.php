...
try {
    $sql = 'select * from repeatsentenceques where repmid = :testid and repqno = :qno';
    $s = $conn->prepare($sql);
...
if (isset($_POST['time'])) {
    if ($_POST['time'] < 0) {
        ...
        try {
            header('location: repeatsentence10.php?x=' . $_POST['time'] . '&y=' . $_POST['ftime']);       
            exit();
        } catch (PDOException $e) {
            echo '<br> error updating database: ' . $e->getMessage();
            exit();
...
<head>
    ...
    <title>PTE Practice Test - Repeat Sentence Question 9</title>
...
$(document).ready(function() {
  ...
  <script>
	...
	var source ="serveruploads/questions/repeatsentence/<?php echo trim($file); ?>";   // enter you audio source here
...
//    }
        ...
        function upload(blob){    
            ...
            xhr('myphpuploaders/repeatsentence9uploader.php', formData, function (fName) {
