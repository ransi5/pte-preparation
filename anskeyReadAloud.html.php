...
try {
    ...
    $s->execute();
    $result = $s->fetchAll();
    $sql1 = "select * from readaloudanswers where reastudid = :studid and reatestid = :testid limit $limit, $perpage";
    $s = $conn->prepare($sql1);
...
<head>
    ...
    <title>PTE Preparation - Read Aloud Answer Key</title>
...
<div class="container-fluid sidenav">
    <div class="row">
        ...
        <div class="col-md-8 mid">
            <h2>Question type: Read Aloud</h2>
            <ul class="pagination">
                <?php for ($i=1; $i <= $pages; $i++) {?>
                <li><a href="?page=<?php echo $i?>">Item <?php echo $i?></a></li>
                <?php } ?>
                <li><a style="background-color: #082f6f; color: white;" href="anskeyRepeatSentence.html.php?page=1">Next Item Type</a></li>
            </ul>
            <?php foreach ($result as $row) {?>
            <div class="qbox">
                <h3>Item <?php echo $row['reaqno'];?></h3>
                <p><?php echo $row['reaquestion'];?></p>
                <?php foreach ($answer as $now) {?>
                <div class="youranswer">
                    <h3>Your Answer</h3>
                    <audio controls>
                        <source src="myphpuploaders/clientuploads/readaloud/<?php echo trim($now['reafile']); ?>" type="audio/wav">
                    </audio>
                </div>
                <?php } ?>
                <div class="modanswer">
                    <h3>Model Answer</h3>
                    <audio controls>
                        <source src="serveruploads/answers/readaloud/<?php echo trim($row['reaanskey']); ?>" type="audio/mpeg">
                    </audio>
                </div>
            </div>
            <?php } ?>
