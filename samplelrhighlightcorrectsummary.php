...
try {
    $sql = 'select * from lrhighlightsummaryques where lrhmid = :testid and lrhqno = :qno';
    $s = $conn->prepare($sql);
    ...
    while ($row = $s->fetch()) {
        $file = $row['lrhfile'];
        $optiona = $row['lrhoptiona'];
        $optionb = $row['lrhoptionb'];
        $optionc = $row['lrhoptionc'];
        $optiond = $row['lrhoptiond'];
    }
...
//        $s->execute();
        if ($_POST['time'] < 0) {
            ...
            try {
                header('location: samplelsmultiplechoice.php?x=' . $_POST['time'] . '&y=' . $_POST['ftime']);       
                exit();
            } catch (PDOException $e) {
                echo '<br> error updating database: ' . $e->getMessage();
                exit();
...
<head>
    ...
    <title>PTE Practice Test - Listening, Highlight Correct Summary Question 1</title>
...
$(document).ready(function(){
  ...
  var audiosource = "serveruploads/questions/lrhighlightcorrectsummary/<?php echo trim($file); ?>";		//	Audio source
