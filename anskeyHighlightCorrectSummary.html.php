...
try {
    ...
    $s->execute();
    $result = $s->fetchAll();
    $sql1 = "select * from lrhighlightsummaryanswers where lrhstudid = :studid and lrhtestid = :testid limit $limit, $perpage";
    $s = $conn->prepare($sql1);
...
<head>
    ...
    <title>PTE Preparation - Highlight Correct Summary Answer Key</title>
...
<div class="container-fluid sidenav">
    <div class="row">
        ...
        <div class="col-md-8 mid">
            <h2>Question type: Highlight Correct Summary</h2>
            <ul class="pagination">
                <?php for ($i=1; $i <= $pages; $i++) {?>
                <li><a href="?page=<?php echo $i?>">Item <?php echo $i?></a></li>
                <?php } ?>
                <li><a style="background-color: #082f6f; color: white;" href="anskeyListeningSingleMCQ.html.php?page=1">Next Item Type</a></li>
            </ul>
            <?php foreach ($result as $row) {?>
            <div class="qbox">
                <h3>Item <?php echo $row['lrhqno'];?></h3>
                <audio controls>
                    <source src="serveruploads/questions/lrhighlightcorrectsummary/<?php echo $row['lrhfile']; ?>" type="audio/mpeg">
                </audio>
                <p>a: <?php echo $row['lrhoptiona'];?></p>
                <p>b: <?php echo $row['lrhoptionb'];?></p>
                <p>c: <?php echo $row['lrhoptionc'];?></p>
                <p>d: <?php echo $row['lrhoptiond'];?></p>
                <?php foreach ($answer as $how) {?>
                <div class="youranswer">
                    <h3>Your Answer</h3>
                    <p class="text"><?php echo $how['lrhanswer'];?></p>
                </div>
                <?php } ?>
                <div class="modanswer">
                    <h3>Model Answer</h3>
                    <p class="text"><?php echo $row['lrhanswer'];?></p>
                </div>
            </div>
            <?php } ?>
