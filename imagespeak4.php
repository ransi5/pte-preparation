...
if (isset($_POST['time'])) {
    if ($_POST['time'] < 0) {
        ...
        try {
            header('location: imagespeak5.php?x=' . $_POST['time'] . '&y=' . $_POST['ftime']);       
            exit();
        } catch (PDOException $e) {
            echo '<br> error updating database: ' . $e->getMessage();
            exit();
...
<head>
    ...
    <title>PTE Practice Test - Describe Image Question 4</title>
...
function successCallback(stream){
    setTimeout(strtrecording,prepare_time*1000+2200);    
        ...
        function upload(blob){    
            ...
            xhr('myphpuploaders/imagespeak4uploader.php', formData, function (fName) {
