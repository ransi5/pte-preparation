...
try {
    $sql = "select * from describeimageques where desmid = :mid";
        $s = $conn->prepare($sql);
...
try {
    $sql = "select * from describeimageanswers where desstudid = :studid and destestid = :testid";
        $s = $conn->prepare($sql);
...
if (isset($_POST['content1'])){
    ...
    $speak = $cont1 + $pron1 + $oral1;
    $readaloud = $cont1 + $pron1 + $oral1 + $speak;
    try {
        $sql = 'update mocktestscores set descontent = descontent + :content, despronounce = despronounce + :pronounce, desfluency = desfluency + :fluency, desspeak = desspeak + :speak, descimgscore = descimgscore + :repsent where mocid = :id';
        $s = $conn->prepare($sql);
        ...
        $s->execute();
        header('location: scoredescribeimage.html.php?x=1');
        exit();
    } catch (PDOException $e) {
...
if (isset($_POST['content2'])){
    ...
    try {
        ...
        $s->bindValue(':id', $_SESSION['mocid']);
        $s->execute();
        header('location: scoredescribeimage.html.php?x=2');
        exit();
    } catch (PDOException $e) {
        echo '<br> error updating database: ' . $e->getMessage();
        exit();
...
if (isset($_POST['content3'])){
    ...
    try {
        ...
        $s->bindValue(':id', $_SESSION['mocid']);
        $s->execute();
        header('location: scoredescribeimage.html.php?x=3');
        exit();
    } catch (PDOException $e) {
        echo '<br> error updating database: ' . $e->getMessage();
        exit();
...
if (isset($_POST['content4'])){
    ...
    try {
        ...
        $s->bindValue(':id', $_SESSION['mocid']);
        $s->execute();
        header('location: scoredescribeimage.html.php?x=4');
        exit();
    } catch (PDOException $e) {
        echo '<br> error updating database: ' . $e->getMessage();
        exit();
...
if (isset($_POST['content5'])){
    ...
    try {
        ...
        $s->bindValue(':id', $_SESSION['mocid']);
        $s->execute();
        header('location: scoredescribeimage.html.php?x=5');
        exit();
    } catch (PDOException $e) {
        echo '<br> error updating database: ' . $e->getMessage();
        exit();
...
if (isset($_POST['content6'])){
    ...
    try {
        ...
        $s->bindValue(':id', $_SESSION['mocid']);
        $s->execute();
        header('location: scoredescribeimage.html.php?x=6');
        exit();
    } catch (PDOException $e) {
        echo '<br> error updating database: ' . $e->getMessage();
        exit();
...
try {
    $sql = 'select descimgscore from mocktestscores where mocid = :id';
    $s = $conn->prepare($sql);
        $s->bindValue(':id', $_SESSION['mocid']);
        $s->execute();
        $col = $s->fetch();
        $score = $col['descimgscore'];
} catch (PDOException $e) {
            echo '<br> error updating database: ' . $e->getMessage();
            exit();
}
        ...
        <div class="gill">
            <div class="container">
                ...
                <table class="quest" style="width: 100%">
                    ...
                    <tr>
                        <td style="text-align: center"><?php echo $row['desmid']; ?></td>
                        <td style="text-align: center"><?php echo $row['desqno']; ?></td>
                        <td><img src="serveruploads/images/imagespeak/<?php echo $row['desfile']; ?>" alt="img" width="100" height="100"></td>
                        <td>
                            <audio controls>
                                <source src="serveruploads/answers/describeimage/<?php echo $row['desanskey']; ?>" type="audio/mpeg">
                            </audio>
                        </td>
                    </tr>
                    <?php }?>
                ...
                <table class="quest" style="width: 100%">
                    ...
                    <tr>
                        <td style="text-align: center"><?php echo $row['destestid']; ?></td>
                        <td style="text-align: center"><?php echo $row['desquesno']; ?></td>
                        <td>
                            <audio controls>
                                <source src="myphpuploaders/clientuploads/imagespeak/<?php echo $row['desfile']; ?>" type="audio/mpeg">
                            </audio>
                        </td>
                        <td>
                            <form action="" method="post" name="MyForm<?php echo $row['desquesno'];?>">
                            Content: <select name="content<?php echo $row['desquesno'];?>">
                                ...
                                Pronunciation: <select name="pronunce<?php echo $row['desquesno'];?>">
                                    <option>Choose Marks</option>
                                    <option value="5">5</option>
                                    <option value="4">4</option>
                                    <option value="3">3</option>
                                ...
                                Oral Fluency: <select name="fluency<?php echo $row['desquesno'];?>">
                                    <option>Choose Marks</option>
                                    <option value="5">5</option>
                                    <option value="4">4</option>
                                    <option value="3">3</option>
                                ...
                                <input type="submit" name="score" value="submit" id="score<?php echo $row['desquesno'];?>">
                            </form>
                        </td>
                    </tr>
                    <?php }?>
                </table>
                <h3>Score: <?php echo $score; ?>/180</h3>
                <table class="quest" style="width: 100%">
                    <tr>
                    ...
                    <tr>
                        ...
                        <td>Describes all elements of the image and their relationships, possible 
                            development and conclusion or implications</td>
                        <td>Native-like</td>
                        <td>Native-like</td>
                    </tr> 
                    <tr>
                        <td style="text-align: center">4</td>
                        <td>Describes all the key elements of the image and their relations, referring 
                            to their implications or conclusions</td>
                        <td>Advanced</td>
                        <td>Advanced</td>
                    </tr>  
                    <tr>
                        <td style="text-align: center">3</td>
                        <td>Deals with most key elements of the image and refers to their implications or conclusions</td>
                        <td>Good</td>
                        <td>Good</td>
                    </tr>  
                    <tr>
                        <td style="text-align: center">2</td>
                        <td>deals with only one key element in the image and refers to an implication or 
                            conclusion. Shows basic understanding of several core elements of the image</td>
                        <td>Intermediate</td>
                        <td>Intermediate</td>
                    </tr>  
                    <tr>
                        <td style="text-align: center">1</td>
                        <td>Describes some basic elements of the image, but does not make clear their 
                            interrelations or implications</td>
                        <td>Intrusive</td>
                        <td>Intrusive</td>
                    </tr>  
                    <tr>
...
$(document).ready(function() {
   $('#next').click(function(){
       window.location = 'scoreretelllecture.html.php?x=';
   });
});
