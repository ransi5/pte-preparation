...
<div class="container">
    <div class="bbox">
        ...
        <div class="confbox">
            <h4>PTE preparation Gold Course - <span class="curr spchar"></span> <span class="price spchar"></span></h4>
            <p>You get</p>
            <p>60 days access - timed practice questions in test like conditions - 6 scored practice test 
            - answer key with model answers - in-depth practice test result & analysis - Complete access to G-Analytics - 
            complete access to PTE strategy & tips on our blog - Email support for upto 21 queries - option to upgrade 
            to Diamond course - FREE tutor feedback on Skype for 1 practice test</p>
        ...
        <div class="buttons">
            <form action="https://www.2checkout.com/checkout/purchase" method="post">
                ...
                <input type='hidden' name='li_0_name' value='Gold Course' >
                <input type='hidden' name='li_0__description' value='60 days access - timed practice questions in test like conditions - 6 scored practice test 
            - answer key with model answers - in-depth practice test result & analysis - complete access to G-Analytics- complete access to 
            PTE strategy & tips on our blog - option to upgrade to Diamond course' >
                <input type="hidden" name="li_0_price" value="" />
                <input type="hidden" name="currency_code" value="" />
                <input type="hidden" name="x_receipt_link_url" value="https://pte-preparation.com/orderconfirmation.html.php?x=3" />
...
$(document).ready(function() {
    $.getJSON("https://freegeoip.net/json/", function (data) {
        ...
        if (country == "India") {
            ...
            $('input[name="li_0_price"]').val('1810.00')
        } else if (country == "Bangladesh") {
            $('.curr').html('USD')
            $('.price').text('26.00')
            $('input[name="currency_code"]').val('USD')
            $('input[name="li_0_price"]').val('26.00')
            ...
            $('.price').text('35')
            $('input[name="currency_code"]').val('USD')
            $('input[name="li_0_price"]').val('35.00')
        } else {
            $('.curr').html('USD')
            $('.price').text('35')
            $('input[name="currency_code"]').val('USD')
