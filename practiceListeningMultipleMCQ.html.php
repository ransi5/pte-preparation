...
try {
    ...
    $sql = "select * from lmcqmultipleanswerques where lmcmid = :mid limit $limit, $perpage";
    $s = $conn->prepare($sql);
    $s->bindValue(':mid', $_SESSION['testid']);
    $s->execute();
    $result = $s->fetchAll();
    $query = $conn->query("select count(*) from lmcqmultipleanswerques where lmcmid = '".$_SESSION['testid']."'");
    $total = $query->fetch();
...
$url = "practiceListeningMultipleMCQ.html.php?page=".$next;
...
<head>
    ...
    <title>PTE Preparation - Practice Listening MCQ, Multiple Answers</title>
...
if (isset($_POST['time'])) {
    try {
        $sql = 'insert into lmcqmultipleansweranswers (lmctestid, lmcquesno, lmcstudid, lmcanswer) values (:test, :quesno, :studid, :content)';
...
<body>
    ...
    <div class="gill">
        <div class="header">
            <p> PTE Listening MCQ Multiple Choice Practice Questions - <?php echo $_SESSION['name'];?></p>
            <div id="cover">
        ...
        <div class="heading">Listen to the recording and answer the question by 
            selecting all the correct responses. You will need to select more than one response.</div>
    ...
    <div class="row">
                <div class="col-md-11">
                    <form action="" method="post" name="MyForm">
                        <p><?php echo trim($row['lmcquestion']); ?></p>
                        <br>
                        <input type="checkbox" name="option[]" value="a"> &nbsp;<?php echo trim($row['lmcoptiona']);?><br><br>
                        <input type="checkbox" name="option[]" value="b"> &nbsp;<?php echo trim($row['lmcoptionb']);?><br><br>
                        <input type="checkbox" name="option[]" value="c"> &nbsp;<?php echo trim($row['lmcoptionc']);?><br><br>
                        <input type="checkbox" name="option[]" value="d"> &nbsp;<?php echo trim($row['lmcoptiond']);?><br><br>
                        <input type="checkbox" name="option[]" value="e"> &nbsp;<?php echo trim($row['lmcoptione']);?><br><br>
...
$(document).ready(function() {
  ...
  <script>
  var preparetime = 7;													//	Prepare time variable
  var audiosource = "serveruploads/questions/lmcqmultipleanswer/<?php echo trim($row['lmcfile']); ?>";		//	Audio source
