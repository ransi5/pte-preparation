...
try {
    $sql = 'select * from lmcqmultipleanswerques where lmcmid = :testid and lmcqno = :qno';
    $s = $conn->prepare($sql);
    ...
    while ($row = $s->fetch()) {
        $file = $row['lmcfile'];
        $question = $row['lmcquestion'];
        $optiona = $row['lmcoptiona'];
        $optionb = $row['lmcoptionb'];
        $optionc = $row['lmcoptionc'];
        $optiond = $row['lmcoptiond'];
        $optione = $row['lmcoptione'];
    }
...
//        $s->execute();
        if ($_POST['time'] < 0) {
            ...
            try {
                header('location: samplelwfillblank.php?x=' . $_POST['time'] . '&y=' . $_POST['ftime']);       
                exit();
            } catch (PDOException $e) {
                echo '<br> error updating database: ' . $e->getMessage();
                exit();
...
<head>
    ...
    <title>PTE Practice Test - Listening, MCQ, Multiple Answers Question 1</title>
...
<body>
  ...
  <script>
    ...
    var time = 900 + <?php if ($_GET['x'] < 0) { echo '('.valid($_GET['x']).')';} else {echo '0';}?>;
...
$(document).ready(function() {
  ...
  var audiosource = "serveruploads/questions/lmcqmultipleanswer/<?php echo trim($file); ?>";		//	Audio source
