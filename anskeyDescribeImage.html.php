...
try {
    ...
    $sql = "select * from describeimageques where desmid = :mid limit $limit, $perpage";
    $s = $conn->prepare($sql);
    $s->bindValue(':mid', $_SESSION['testid']);
    $s->execute();
    $result = $s->fetchAll();
    $sql1 = "select * from describeimageanswers where desstudid = :studid and destestid = :testid limit $limit, $perpage";
    $s = $conn->prepare($sql1);
    $s->bindValue(':studid', $_SESSION['id']);
    $s->bindValue(':testid', $_SESSION['testid']);
    $s->execute();
    $answer = $s->fetchAll();
    $query = $conn->query("select count(*) from describeimageques where desmid = '".$_SESSION['testid']."'");
    $total = $query->fetch();
...
<head>
    ...
    <title>PTE Preparation - Describe Image Answer Key</title>
...
<div class="container-fluid sidenav">
    <div class="row">
        ...
        <div class="col-md-8 mid">
            <h2>Question type: Describe Image</h2>
            <ul class="pagination">
                <?php for ($i=1; $i <= $pages; $i++) {?>
                <li><a href="?page=<?php echo $i?>">Item <?php echo $i?></a></li>
                <?php } ?>
                <li><a style="background-color: #082f6f; color: white;" href="anskeyRe-tellLecture.html.php?page=1">Next Item Type</a></li>
            </ul>
            <?php foreach ($result as $row) {?>
            <div class="qbox">
                <h3>Item <?php echo $row['desqno'];?></h3>
                <img src="serveruploads/images/imagespeak/<?php echo $row['desfile'];?>" class="img-responsive" alt="img">
                <?php foreach ($answer as $how) {?>
                <div class="youranswer">
                    <h3>Your Answer</h3>
                    <audio controls>
                        <source src="myphpuploaders/clientuploads/imagespeak/<?php echo $how['desfile']; ?>" type="audio/mpeg">
                    </audio>
                </div>
                <?php } ?>
                <div class="modanswer">
