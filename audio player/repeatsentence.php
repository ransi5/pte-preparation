...
session_start();
include 'connect.php';
$sth = $db->prepare("SELECT * FROM repeatsentence");
$sth->execute();
$numofrows=$sth->rowCount();

$prestring="serveruploads/questions/repeatsentence/";
while ($row = $sth->fetch(PDO::FETCH_ASSOC)) 
{
   $prepare_time[] = $row['prepare_time'];
   $recording_time[]= $row['recording_time'];
   $source[] = $prestring."".$row['file_name'].".mp3";
...
<body>
	...
	<div class="container">
		<div class="row">
   			<p class="text-justify" ><strong><i>You will hear a sentence. Please repeat the sentence exactly as you hear it. You will hear the sentence only once.</i></strong></p>
        </div>
		<br>
		<br>
		<br>
  ...
  <script>
	...
	var sourcearr = <?php echo json_encode($source); ?>;
  ...
  function main()
  {
	...
	{	
		...
		timeelapsed.innerHTML="-";
				...
				var x=prepare_timearr[i];
				var y=sourcearr[i];
				var z=recording_timearr[i];
				secondtomain(x,y,z);
...
function startRecording() {
 ...
 //   button.nextElementSibling.disabled = false;
  ...
  <script src="mylibs/repeatsentencerecordmp3.js"></script>
</body>
</html>