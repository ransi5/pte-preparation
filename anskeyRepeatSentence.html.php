...
try {
    ...
    $s->execute();
    $result = $s->fetchAll();
    $sql1 = "select * from repeatsentenceanswers where repstudid = :studid and reptestid = :testid limit $limit, $perpage";
    $s = $conn->prepare($sql1);
...
<head>
    ...
    <title>PTE Preparation - Repeat Sentence Answer Key</title>
...
<div class="container-fluid sidenav">
    <div class="row">
        ...
        <div class="col-md-8 mid">
            <h2>Question type: Repeat Sentence</h2>
            <ul class="pagination">
                <?php for ($i=1; $i <= $pages; $i++) {?>
                <li><a href="?page=<?php echo $i?>">Item <?php echo $i?></a></li>
                <?php } ?>
                <li><a style="background-color: #082f6f; color: white;" href="anskeyDescribeImage.html.php?page=1">Next Item Type</a></li>
            </ul>
            <?php foreach ($result as $row) {?>
            <div class="qbox">
                <h3>Item <?php echo $row['repqno'];?></h3>
                <audio controls>
                    <source src="serveruploads/questions/repeatsentence/<?php echo $row['repfile']; ?>" type="audio/mpeg">
                </audio>
                <?php foreach ($answer as $how) {?>
                <div class="youranswer">
                    <h3>Your Answer</h3>
                    <audio controls>
                        <source src="myphpuploaders/clientuploads/repeatsentence/<?php echo $how['repfile']; ?>" type="audio/mpeg">
                    </audio>
                </div>
                <?php } ?>
                <div class="modanswer">
                    <h3>Model Answer</h3>
                    <p><?php echo $row['repquestion']; ?></p>
                </div>
            </div>
            <?php } ?>
