...
try {
    $sql = 'select * from lwfillblankques where lwfmid = :testid and lwfqno = :qno';
    $s = $conn->prepare($sql);
...
//        $s->execute();
        if ($_POST['time'] < 0) {
            ...
            try {
                header('location: samplelrhighlightcorrectsummary.php?x=' . $_POST['time'] . '&y=' . $_POST['ftime']);       
                exit();
            } catch (PDOException $e) {
                echo '<br> error updating database: ' . $e->getMessage();
                exit();
...
<head>
    ...
    <title>PTE Practice Test - Listening & Writing, Fill in the Blanks Question 1</title>
...
$(document).ready(function() {
  ...
  var audiosource = "serveruploads/questions/lwfillblank/<?php echo trim($file); ?>";		//	Audio source
