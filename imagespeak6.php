...
if (isset($_POST['time'])) {
    if ($_POST['time'] < 0) {
        ...
        try {
            header('location: retelllecture.php?x=' . $_POST['time'] . '&y=' . $_POST['ftime']);       
            exit();
        } catch (PDOException $e) {
            echo '<br> error updating database: ' . $e->getMessage();
            exit();
...
<head>
    ...
    <title>PTE Practice Test - Describe Image Question 6</title>
...
function successCallback(stream){
    setTimeout(strtrecording,prepare_time*1000+2200);    
        ...
        function upload(blob){    
            ...
            xhr('myphpuploaders/imagespeak6uploader.php', formData, function (fName) {
