...
try {
    ...
    $s->execute();
    $result = $s->fetchAll();
    $sql1 = "select * from lselectmissingwordanswers where lsestudid = :studid and lsetestid = :testid limit $limit, $perpage";
    $s = $conn->prepare($sql1);
...
<head>
    ...
    <title>PTE Preparation - Select Missing Word Answer Key</title>
...
<div class="container-fluid sidenav">
    <div class="row">
        ...
        <div class="col-md-8 mid">
            <h2>Question type: Select Missing Word</h2>
            <ul class="pagination">
                <?php for ($i=1; $i <= $pages; $i++) {?>
                <li><a href="?page=<?php echo $i?>">Item <?php echo $i?></a></li>
                <?php } ?>
                <li><a style="background-color: #082f6f; color: white;" href="anskeyHighlightIncorrectWords.html.php?page=1">Next Item Type</a></li>
            </ul>
            <?php foreach ($result as $row) {?>
            <div class="qbox">
                <h3>Item <?php echo $row['lseqno'];?></h3>
                <audio controls>
                    <source src="serveruploads/questions/lselectmissingword/<?php echo $row['lsefile']; ?>" type="audio/mpeg">
                </audio>
                <p>a: <?php echo $row['lseoptiona'];?></p>
                <p>b: <?php echo $row['lseoptionb'];?></p>
                <p>c: <?php echo $row['lseoptionc'];?></p>
                <p>d: <?php echo $row['lseoptiond'];?></p>
                <p>e: <?php echo $row['lseoptione'];?></p>
                <?php foreach ($answer as $how) {?>
                <div class="youranswer">
                    <h3>Your Answer</h3>
                    <p class="text"><?php echo $how['lseanswer'];?></p>
                </div>
                <?php } ?>
                <div class="modanswer">
                    <h3>Model Answer</h3>
                    <p class="text"><?php echo $row['lseanswer'];?></p>
                </div>
            </div>
            <?php } ?>
