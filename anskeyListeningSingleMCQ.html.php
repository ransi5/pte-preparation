...
try {
    ...
    $s->execute();
    $result = $s->fetchAll();
    $sql1 = "select * from lsmultiplechoiceanswers where lsmstudid = :studid and lsmtestid = :testid limit $limit, $perpage";
    $s = $conn->prepare($sql1);
...
<head>
    ...
    <title>PTE Preparation - Listening MCQ, Single Answer - Answer Key</title>
...
<div class="container-fluid sidenav">
    <div class="row">
        ...
        <div class="col-md-8 mid">
            <h2>Question type: Listening MCQ, Single Answer</h2>
            <ul class="pagination">
                <?php for ($i=1; $i <= $pages; $i++) {?>
                <li><a href="?page=<?php echo $i?>">Item <?php echo $i?></a></li>
                <?php } ?>
                <li><a style="background-color: #082f6f; color: white;" href="anskeySelectMissingWord.html.php?page=1">Next Item Type</a></li>
            </ul>
            <?php foreach ($result as $row) {?>
            <div class="qbox">
                <h3>Item <?php echo $row['lsmqno'];?></h3>
                <audio controls>
                    <source src="serveruploads/questions/lsmultiplechoice/<?php echo $row['lsmfile']; ?>" type="audio/mpeg">
                </audio>
                <p>a: <?php echo $row['lsmquestion'];?></p>
                <p>a: <?php echo $row['lsmoptiona'];?></p>
                <p>b: <?php echo $row['lsmoptionb'];?></p>
                <p>c: <?php echo $row['lsmoptionc'];?></p>
                <p>d: <?php echo $row['lsmoptiond'];?></p>
                <?php foreach ($answer as $how) {?>
                <div class="youranswer">
                    <h3>Your Answer</h3>
                    <p class="text"><?php echo $how['lsmanswer'];?></p>
                </div>
                <?php } ?>
                <div class="modanswer">
                    <h3>Model Answer</h3>
                    <p class="text"><?php echo $row['lsmanswer'];?></p>
                </div>
            </div>
            <?php } ?>
