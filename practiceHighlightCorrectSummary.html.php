...
try {
    ...
    $sql = "select * from lrhighlightsummaryques where lrhmid = :mid limit $limit, $perpage";
    $s = $conn->prepare($sql);
    $s->bindValue(':mid', $_SESSION['testid']);
    $s->execute();
    $result = $s->fetchAll();
    $query = $conn->query("select count(*) from lrhighlightsummaryques where lrhmid = '".$_SESSION['testid']."'");
    $total = $query->fetch();
...
$url = "practiceHighlightCorrectSummary.html.php?page=".$next;
...
<head>
    ...
    <title>PTE Preparation - Practice Highlight Correct Summary</title>
...
if (isset($_POST['time'])) {
    try {
        $sql = 'insert into lrhighlightsummaryanswers (lrhtestid, lrhquesno, lrhstudid, lrhanswer) values (:test, :quesno, :studid, :content)';
        $s = $conn->prepare($sql);
...
<body>
    ...
    <div class="gill">
        <div class="header">
            <p> PTE Highlight Correct Summary Practice Questions - <?php echo $_SESSION['name'];?></p>
            <div id="cover">
        ...
        <div class="heading">You will hear a recording. Click on the paragraph that best relates to the recording.</div>
    ...
    <div class="row">
                <div class="col-md-11">
                    <form action="" method="post" name="MyForm">
                        <input type="radio" name="options" value="a"><span class="high"> &nbsp;<?php echo trim($row['lrhoptiona']);?></span><br><br>
                        <input type="radio" name="options" value="b"><span class="high"> &nbsp;<?php echo trim($row['lrhoptionb']);?></span><br><br>
                        <input type="radio" name="options" value="c"><span class="high"> &nbsp;<?php echo trim($row['lrhoptionc']);?></span><br><br>
                        <input type="radio" name="options" value="d"><span class="high"> &nbsp;<?php echo trim($row['lrhoptiond']);?></span><br><br>
...
$(document).ready(function(){
  ...
  var audiosource = "serveruploads/questions/lrhighlightcorrectsummary/<?php echo trim($row['lrhfile']); ?>";		//	Audio source
