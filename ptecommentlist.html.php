...
try {
    $sql = "select * from comments where comstatus = :status order by date";
        $s = $conn->prepare($sql);
...
if (isset($_POST['action']) && $_POST['action'] == 'Approve') {
    try {
    $sql = "update comments set comstatus = :status where cid = :id";
        $s = $conn->prepare($sql);
        $s->bindValue(':id', $_POST['id']);
        $s->bindValue(':status', 'Approved');
        $s->execute();      
        header('location: ptecommentlist.html.php');
        exit();
    } catch (PDOException $e) {
        echo '<br>Error fetching question: ' . $e->getMessage();
        exit();
...
<head>
    ...
    <title>PTE Preparation - Comments List</title>
