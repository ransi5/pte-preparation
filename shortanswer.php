...
if (isset($_POST['time'])) {
    if ($_POST['time'] < 0) {
        ...
        try {
            header('location: shortanswer2.php?x=' . $_POST['time'] . '&y=' . $_POST['ftime']);       
            exit();
        } catch (PDOException $e) {
            echo '<br> error updating database: ' . $e->getMessage();
            exit();
...
<head>
    ...
    <title>PTE Practice Test - Answer Short Question 1</title>
...
//    }
        ...
        function upload(blob){    
            ...
            xhr('myphpuploaders/shortansweruploader.php', formData, function (fName) {
