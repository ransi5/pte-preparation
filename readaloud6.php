...
try {
    $sql = "select * from readaloudques where reamid = :mid and reaqno = :qno";
        $s = $conn->prepare($sql);
        $s->bindValue(':mid', $_SESSION['testid']);
        $s->bindValue(':qno', '6');
        $s->execute();
        $result = $s->fetch();
        $question = $result['reaquestion'];
...
if (isset($_POST['time'])) {
    if ($_POST['time'] < 0) {
        ...
        try {
            header('location: repeatsentence.php?x=' . $_POST['time'] . '&y=' . $_POST['ftime']);       
            exit();
        } catch (PDOException $e) {
            echo '<br> error updating database: ' . $e->getMessage();
            exit();
...
<head>
    ...
    <title>PTE Practice Test - Read Aloud Question 6</title>
...
<body>
	...
	<br>
        ...
        <br>
    <div class="row">
        <p id="passage" class="text-justify"><?php echo $question ?></p>
...
function successCallback(stream){
    setTimeout(strtrecording,prepare_time*1000+2000);    
        ...
        function upload(blob){    
            ...
            xhr('myphpuploaders/readaloud6uploader.php', formData, function (fName) {
