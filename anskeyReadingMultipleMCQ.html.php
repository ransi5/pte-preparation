...
try {
    ...
    $s->execute();
    $result = $s->fetchAll();
    $sql1 = "select * from mmcreadinganswers where mmcstudid = :studid and mmctestid = :testid limit $limit, $perpage";
    $s = $conn->prepare($sql1);
...
<head>
    ...
    <title>PTE Preparation - Reading MCQ, Multiple Answers, Answer Key</title>
...
<div class="container-fluid sidenav">
    <div class="row">
        ...
        <div class="col-md-8 mid">
            <h2>Question type: MCQ, Multiple Answer</h2>
            <ul class="pagination">
                <?php for ($i=1; $i <= $pages; $i++) {?>
                <li><a href="?page=<?php echo $i?>">Item <?php echo $i?></a></li>
                <?php } ?>
                <li><a style="background-color: #082f6f; color: white;" href="anskeyRe-orderParagraphs.html.php?page=1">Next Item Type</a></li>
            </ul>
            <?php foreach ($result as $row) {?>
            <div class="qbox">
                <h3>Item <?php echo $row['mmcqno'];?></h3>
                <p><?php echo $row['mmccontent'];?></p>
                <p><?php echo $row['mmcquestion'];?></p>
                <p>a: <?php echo $row['mmcoptiona'];?></p>
                <p>b: <?php echo $row['mmcoptionb'];?></p>
                <p>c: <?php echo $row['mmcoptionc'];?></p>
                <p>d: <?php echo $row['mmcoptiond'];?></p>
                <p>e: <?php echo $row['mmcoptione'];?></p>
                <?php foreach ($answer as $how) {?>
                <div class="youranswer">
                    <h3>Your Answer</h3>
                    <p class="text"><?php echo $how['mmcanswer'];?></p>
                </div>
                <?php } ?>
                <div class="modanswer">
                    <h3>Model Answer</h3>
                    <p class="text"><?php echo $row['mmcanskey'];?></p>
                </div>
            </div>
            <?php } ?>
