...
try {
    ...
    $s->execute();
    $result = $s->fetchAll();
    $sql1 = "select * from lwsummarizespokentextanswers where lwsstudid = :studid and lwstestid = :testid limit $limit, $perpage";
    $s = $conn->prepare($sql1);
...
<head>
    ...
    <title>PTE Preparation - Summarize Spoken Text Answer Key</title>
...
<div class="container-fluid sidenav">
    <div class="row">
        ...
        <div class="col-md-8 mid">
            <h2>Question type: Summarize Spoken Text</h2>
            <ul class="pagination">
                <?php for ($i=1; $i <= $pages; $i++) {?>
                <li><a href="?page=<?php echo $i?>">Item <?php echo $i?></a></li>
                <?php } ?>
                <li><a style="background-color: #082f6f; color: white;" href="anskeyListeningMultipleMCQ.html.php?page=1">Next Item Type</a></li>
            </ul>
            <?php foreach ($result as $row) {?>
            <div class="qbox">
                <h3>Item <?php echo $row['lwsqno'];?></h3>
                <audio controls>
                    <source src="serveruploads/questions/lwsummarizespokentext/<?php echo $row['lwsfile']; ?>" type="audio/mpeg">
                </audio>
                <?php foreach ($answer as $how) {?>
                <div class="youranswer">
                    <h3>Your Answer</h3>
                    <p class="text"><?php echo $how['lwscontentans'];?></p>
                </div>
                <?php } ?>
                <div class="modanswer">
                    <h3>Model Answer</h3>
                    <p class="text"><?php echo $row['lwsanswer'];?></p>
                </div>
            </div>
            <?php } ?>
