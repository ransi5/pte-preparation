...
try {
    ...
    $sql = "select * from retelllectureques where retmid = :mid limit $limit, $perpage";
    $s = $conn->prepare($sql);
    $s->bindValue(':mid', $_SESSION['testid']);
    $s->execute();
    $result = $s->fetchAll();
    $query = $conn->query("select count(*) from retelllectureques where retmid = '".$_SESSION['testid']."'");
    $total = $query->fetch();
...
<head>
    ...
    <title>PTE Preparation - Practice Re-Tell Lecture</title>
...
<body>
    ...
    <div id="row">
        ...
        <ul id="recordingslist" type="Hidden"></ul>
                ...
                <div id="footer" class="bottom"><a href="practiceRe-tellLecture.html.php?page=<?php echo $_GET['page']+1; ?>">Next</a></div>
    </div>
    </div>
...
<script>
	...
	var audiosource = "serveruploads/questions/retelllecture/<?php echo trim($row['retfile']);?>";	//enter audiosource here
	var imagesource ="serveruploads/images/retelllecture/<?php echo trim($row['retimage']);?>";	//enter image source here
...
//        setTimeout(strtrecording,audiodurt+12000);
//    }
        ...
        function upload(blob){    
            ...
            var fileName = '<?php echo $_SESSION['id'] . $_SESSION['testid'] . $_SESSION['name'] ."-". date( 'd-m-Y H-i-s' );?>' + '.wav'  // or "wav"

            var formData = new FormData();
            formData.append(fileType + '-filename', fileName);
            formData.append(fileType + '-blob', blob);

            xhr('myphpuploaders/retelllecture11uploader.php', formData, function (fName) {
