...
session_start();
include 'connect.php';
$sth = $db->prepare("SELECT * FROM readaloud");
$sth->execute();
$numofrows=$sth->rowCount();
$prestring="serveruploads/textfiles/readaloud/";
while ($row = $sth->fetch(PDO::FETCH_ASSOC)) {
   $prepare_time[] = $row['prepare_time'];
   $source[] = $prestring."".$row['file_name'].".txt";
   
   $questionidarr[]=$row['id'];
   
   $filepath = $prestring."".$row['file_name'].".txt";
   $myfile = fopen($filepath, "r") or die("Unable to open file!");
   $filedata=fread($myfile,filesize($filepath));
   $filedataarr[]=$filedata;
   $wordcount = count(explode(" ", $filedata));
   $wordcountarr[]=$wordcount;
   fclose($myfile);
}
?>
<?php
...
<body>
  ...
  <script>
	...
	var filedataarr = <?php echo json_encode($filedataarr); ?>;
	var wordcountarr = <?php echo json_encode($wordcountarr); ?>;
	...
	function initiator()
		{	
			...
			{
				i++;
				printer(prepare_timearr[i],filedataarr[i],wordcountarr[i]);
				questionno++;
			}
