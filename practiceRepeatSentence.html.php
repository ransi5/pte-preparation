...
try {
    ...
    $sql = "select * from repeatsentenceques where repmid = :mid limit $limit, $perpage";
    $s = $conn->prepare($sql);
    $s->bindValue(':mid', $_SESSION['testid']);
    $s->execute();
    $result = $s->fetchAll();
    $query = $conn->query("select count(*) from repeatsentenceques where repmid = '".$_SESSION['testid']."'");
    $total = $query->fetch();
...
<head>
    ...
    <title>PTE Preparation - Practice Repeat Sentence</title>
...
<body>
    ...
    <div class="gill">
        <div class="header">
            <p> PTE Repeat Sentence Practice Questions - <?php echo $_SESSION['name'];?></p>
            <div id="cover">
...
<div id="footer" class="bottom1"><a href="practiceRepeatSentence.html.php?page=<?php echo $_GET['page']+1; ?>">Next</a></div>
</div>
</div>
  <script>
var time = 30;
...
$(document).ready(function() {
  ...
  <script>
	...
	var recording_time = 15;  //enter recordingtime here
	var source ="serveruploads/questions/repeatsentence/<?php echo trim($row['repfile']); ?>";   // enter you audio source here
	var numofrows = 1;					//enter when you have multiple questions
	//getters
	...
	function nextpage()
	{
		window.location.href = 'imagespeak.php';
	}
	var audio;
        audio=new Audio();
        audio.src=source;
  ...
  function audiostarter()
  {
	  ...
	  {
		  counter--;
		  audiostatus.innerHTML="<p>Playing in <strong>"+counter+"</strong> seconds</p>";
	  }
	  else
	  {
...
//        setTimeout(strtrecording,audiodurt);
//    }
        ...
        function upload(blob){    
            ...
            xhr('myphpuploaders/repeatsentence11uploader.php', formData, function (fName) {
...
//    var mediaConstraints = { audio: true };
//    setTimeout(function(){navigator.mediaDevices.getUserMedia(mediaConstraints).then(successCallback).catch(errorCallback);},1000);
