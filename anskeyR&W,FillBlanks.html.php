...
try {
    ...
    $s->execute();
    $result = $s->fetchAll();
    $sql1 = "select * from rwfillblankanswers where rwfstudid = :studid and rwftestid = :testid limit $limit, $perpage";
    $s = $conn->prepare($sql1);
...
<head>
    ...
    <title>PTE Preparation - Reading & Writing, Fill in the Blanks Answer Key</title>
...
<div class="container-fluid sidenav">
    <div class="row">
        ...
        <div class="col-md-8 mid">
            <h2>Question type: Reading and Writing, Fill in the Blanks</h2>
            <ul class="pagination">
                <?php for ($i=1; $i <= $pages; $i++) {?>
                <li><a href="?page=<?php echo $i?>">Item <?php echo $i?></a></li>
                <?php } ?>
                <li><a style="background-color: #082f6f; color: white;" href="anskeySummarizeSpokenText.html.php?page=1">Next Item Type</a></li>
            </ul>
            <?php foreach ($result as $row) {?>
            <div class="qbox">
                <h3>Item <?php echo $row['rwfqno'];?></h3>
                <p><?php echo $row['rwfcontent'];?></p>
                <?php foreach ($answer as $how) {?>
                <div class="youranswer">
                    <h3>Your Answer</h3>
                    <p class="text"><?php echo $how['rwfansa'];?>, <?php echo $how['rwfansb'];?>, 
                    <?php echo $how['rwfansc'];?>, <?php echo $how['rwfansd'];?></p>
                </div>
                <?php } ?>
                <div class="modanswer">
                    <h3>Model Answer</h3>
                    <p class="text"><?php echo $row['rwfanswera'];?>, <?php echo $row['rwfanswerb'];?>, <?php echo $row['rwfanswerc'];?>
                    , <?php echo $row['rwfanswerd'];?></p>
                </div>
            </div>
            <?php } ?>
