...
foreach(array('audio') as $type) {
    if (isset($_FILES["${type}-blob"])) {
        ...
        $uploadDirectory = 'clientuploads/retelllecture/'.$filename;
...
try {
    $sql = 'insert into retelllectureanswers (rettestid, retquesno, retstudid, retfile) values (:test, :quesno, :studid, :file)';
    $s = $conn->prepare($sql);
