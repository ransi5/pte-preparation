...
try {
    $sql = 'select * from lwsummarizespokentextques where lwsmid = :testid and lwsqno = :qno';
    $s = $conn->prepare($sql);
...
//        $s->execute();
        header('location: samplelmcqmultipleanswer.php?x=' . $_POST['time'] . '&y=' . $_POST['ftime']);       
        exit();       
    } catch (PDOException $e) {
...
<head>
    ...
    <title>PTE Practice Test - Summarize Spoken Text Question 1</title>
...
$(document).ready(function() {
  ...
  var audiosource = "serveruploads/questions/lwsummarizespokentext/<?php echo trim($file); ?>";		//	Audio source
