...
try {
    $sql = 'select * from lselectmissingwordques where lsemid = :testid and lseqno = :qno';
    $s = $conn->prepare($sql);
    $s->bindValue(':testid', '111');
    $s->bindValue(':qno', '1');
    $s->execute();
    while ($row = $s->fetch()) {
        $file = $row['lsefile'];
        $optiona = $row['lseoptiona'];
        $optionb = $row['lseoptionb'];
        $optionc = $row['lseoptionc'];
        $optiond = $row['lseoptiond'];
        $optione = $row['lseoptione'];
    }
...
//        $s->execute();
        if ($_POST['time'] < 0) {
            ...
            try {
                header('location: samplelhighlightincorrectword.php?x=' . $_POST['time'] . '&y=' . $_POST['ftime']);       
                exit();
            } catch (PDOException $e) {
                echo '<br> error updating database: ' . $e->getMessage();
                exit();
...
<head>
    ...
    <title>PTE Practice Test - Select Missing Word Question 1</title>
    ...
    <div class="row">
                <div class="col-md-11">
                    <form action="" method="post" name="MyForm">
                        ...
                        <input type="radio" name="options" value="e"><span class="high"> &nbsp;<?php echo trim($optione)?></span><br><br>
...
$(document).ready(function(){
  ...
  var audiosource = "serveruploads/questions/lselectmissingword/<?php echo trim($file); ?>";		//	Audio source
