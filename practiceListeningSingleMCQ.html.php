...
try {
    ...
    $sql = "select * from lsmultiplechoiceques where lsmmid = :mid limit $limit, $perpage";
    $s = $conn->prepare($sql);
    $s->bindValue(':mid', $_SESSION['testid']);
    $s->execute();
    $result = $s->fetchAll();
    $query = $conn->query("select count(*) from lsmultiplechoiceques where lsmmid = '".$_SESSION['testid']."'");
    $total = $query->fetch();
...
$url = "practiceListeningSingleMCQ.html.php?page=".$next;
...
<body>
    ...
    <div class="container-fluid">
        <div class="gill">
        <div class="header">
            <p> PTE Listening MCQ Single Answer Practice Questions - <?php echo $_SESSION['name'];?></p>
            <div id="cover">
	...
	<div class="row">
        ...
        <br>
    <div class="container">
        <div class="question"><?php echo trim($row['lsmquestion']); ?></div><br>
	
    <div class="row">
                <div class="col-md-11">
                    <form action="" method="post" name="MyForm">
                        <input type="radio" name="options" value="a"><span class="high"> &nbsp;<?php echo trim($row['lsmoptiona']);?></span><br><br>
                        <input type="radio" name="options" value="b"><span class="high"> &nbsp;<?php echo trim($row['lsmoptionb']);?></span><br><br>
                        <input type="radio" name="options" value="c"><span class="high"> &nbsp;<?php echo trim($row['lsmoptionc']);?></span><br><br>
                        <input type="radio" name="options" value="d"><span class="high"> &nbsp;<?php echo trim($row['lsmoptiond']);?></span><br><br>
...
$(document).ready(function(){
  ...
  var audiosource = "serveruploads/questions/lsmultiplechoice/<?php echo trim($row['lsmfile']); ?>";		//	Audio source
