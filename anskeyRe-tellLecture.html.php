...
try {
    ...
    $s->execute();
    $result = $s->fetchAll();
    $sql1 = "select * from retelllectureanswers where retstudid = :studid and rettestid = :testid limit $limit, $perpage";
    $s = $conn->prepare($sql1);
...
<head>
    ...
    <title>PTE Preparation - Re-Tell Lecture Answer Key</title>
...
<div class="container-fluid sidenav">
    <div class="row">
        ...
        <div class="col-md-8 mid">
            <h2>Question type: Re-tell Lecture</h2>
            <ul class="pagination">
                <?php for ($i=1; $i <= $pages; $i++) {?>
                <li><a href="?page=<?php echo $i?>">Item <?php echo $i?></a></li>
                <?php } ?>
                <li><a style="background-color: #082f6f; color: white;" href="anskeyAnswerShortQuestion.html.php?page=1">Next Item Type</a></li>
            </ul>
            <?php foreach ($result as $row) {?>
            <div class="qbox">
                <h3>Item <?php echo $row['retqno'];?></h3>
                <audio controls>
                    <source src="serveruploads/questions/retelllecture/<?php echo $row['retfile']; ?>" type="audio/mpeg">
                </audio>
                <?php foreach ($answer as $how) {?>
                <div class="youranswer">
                    <h3>Your Answer</h3>
                    <audio controls>
                        <source src="myphpuploaders/clientuploads/retelllecture/<?php echo $how['retfile']; ?>" type="audio/mpeg">
                    </audio>
                </div>
                <?php } ?>
                <div class="modanswer">
