...
if (isset($_POST['time'])) {
    if ($_POST['time'] < 0) {
        ...
        try {
            header('location: readaloud6.php?x=' . $_POST['time'] . '&y=' . $_POST['ftime']);       
            exit();
        } catch (PDOException $e) {
            echo '<br> error updating database: ' . $e->getMessage();
            exit();
...
<head>
    ...
    <title>PTE Practice Test - Read Aloud Question 5</title>
...
function successCallback(stream){
    setTimeout(strtrecording,prepare_time*1000+2000);    
        ...
        function upload(blob){    
            ...
            xhr('myphpuploaders/readaloud5uploader.php', formData, function (fName) {
