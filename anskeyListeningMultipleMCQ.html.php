...
try {
    ...
    $s->execute();
    $result = $s->fetchAll();
    $sql1 = "select * from lmcqmultipleansweranswers where lmcstudid = :studid and lmctestid = :testid limit $limit, $perpage";
    $s = $conn->prepare($sql1);
...
<head>
    ...
    <title>PTE Preparation - Listening MCQ, Multiple Answers Answer Key</title>
...
<div class="container-fluid sidenav">
    <div class="row">
        ...
        <div class="col-md-8 mid">
            <h2>Question type: Listening MCQ, Multiple Choice</h2>
            <ul class="pagination">
                <?php for ($i=1; $i <= $pages; $i++) {?>
                <li><a href="?page=<?php echo $i?>">Item <?php echo $i?></a></li>
                <?php } ?>
                <li><a style="background-color: #082f6f; color: white;" href="anskeyL&W,FillBlanks.html.php?page=1">Next Item Type</a></li>
            </ul>
            <?php foreach ($result as $row) {?>
            <div class="qbox">
                <h3>Item <?php echo $row['lmcqno'];?></h3>
                <audio controls>
                    <source src="serveruploads/questions/lmcqmultipleanswer/<?php echo $row['lmcfile']; ?>" type="audio/mpeg">
                </audio>
                <p><?php echo $row['lmcquestion'];?></p>
                <p>a: <?php echo $row['lmcoptiona'];?></p>
                <p>b: <?php echo $row['lmcoptionb'];?></p>
                <p>c: <?php echo $row['lmcoptionc'];?></p>
                <p>d: <?php echo $row['lmcoptiond'];?></p>
                <p>e: <?php echo $row['lmcoptione'];?></p>
                <?php foreach ($answer as $how) {?>
                <div class="youranswer">
                    <h3>Your Answer</h3>
                    <p class="text"><?php echo $how['lmcanswer'];?></p>
                </div>
                <?php } ?>
                <div class="modanswer">
                    <h3>Model Answer</h3>
                    <p class="text"><?php echo $row['lmcanswer'];?></p>
                </div>
            </div>
            <?php } ?>
