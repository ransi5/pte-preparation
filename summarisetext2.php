...
try {
    $sql = 'select * from summarisewrittenques where summid = :testid and sumqno = :qno';
    $s = $conn->prepare($sql);
    $s->bindValue(':testid', $_SESSION['testid']);
    $s->bindValue(':qno', '2');
    $s->execute();
    while ($row = $s->fetch()) {
        $content = $row['sumcontent'];
    }
...
if (isset($_POST['content'])) {
    try {
        ...
        $s->execute();
        header('location: essay.php?x=' . $_POST['time'] . '&y=' . $_POST['ftime']);
        exit();
    } catch (PDOException $e) {
...
<head>
    ...
    <title>PTE Practice Test - Summarize Written Text Question 2</title>
...
$(document).ready(function() {
   ...
   console.log('window: ' + docHeight);
   console.log('gill: ' + footerTop)
