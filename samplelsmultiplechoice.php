...
try {
    $sql = 'select * from lsmultiplechoiceques where lsmmid = :testid and lsmqno = :qno';
    $s = $conn->prepare($sql);
    $s->bindValue(':testid', '6');
    $s->bindValue(':qno', '2');
    $s->execute();
    while ($row = $s->fetch()) {
        $file = $row['lsmfile'];
        $question = $row['lsmquestion'];
        $optiona = $row['lsmoptiona'];
        $optionb = $row['lsmoptionb'];
        $optionc = $row['lsmoptionc'];
        $optiond = $row['lsmoptiond'];
    }
...
if (isset($_POST['time'])) {
    try {
        $sql = 'insert into lsmultiplechoiceanswers (lsmtestid, lsmquesno,  lsmstudid, lsmanswer) values (:test, :quesno, :studid, :content)';
        $s = $conn->prepare($sql);
        ...
        if ($_POST['time'] < 0) {
            try {
                header('location: samplemocktestend.php');       
                exit();
            } catch (PDOException $e) {
            ...
            try {
                header('location: samplelselectmissingword.php?x=' . $_POST['time'] . '&y=' . $_POST['ftime']);       
                exit();
            } catch (PDOException $e) {
                echo '<br> error updating database: ' . $e->getMessage();
                exit();
...
<head>
    ...
    <title>PTE Practice Test - Listening, MCQ, Single Answer Question 1</title>
...
<body>
    ...
    <div class="container-fluid">
        ...
        <div class="heading">Listen to the recording and answer the multiple-choice question by
         selecting the correct response. Only one response is correct.</div>
	...
	<div class="row">
        ...
        <br>
    <div class="container">
        <div class="question"><?php echo trim($question); ?></div><br>
	
    <div class="row">
                <div class="col-md-11">
                    <form action="" method="post" name="MyForm">
                        <input type="radio" name="options" value="a"><span class="high"> &nbsp;<?php echo trim($optiona)?></span><br><br>
                        <input type="radio" name="options" value="b"><span class="high"> &nbsp;<?php echo trim($optionb)?></span><br><br>
                        <input type="radio" name="options" value="c"><span class="high"> &nbsp;<?php echo trim($optionc)?></span><br><br>
                        <input type="radio" name="options" value="d"><span class="high"> &nbsp;<?php echo trim($optiond)?></span><br><br>
...
$(document).ready(function(){
  ...
  <script>
  var preparetime = 6;											//	Prepare time variable
  var audiosource = "serveruploads/questions/lsmultiplechoice/<?php echo trim($file); ?>";		//	Audio source
