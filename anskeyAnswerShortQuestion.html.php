...
try {
    ...
    $sql = "select * from shortanswerques where shomid = :mid limit $limit, $perpage";
    $s = $conn->prepare($sql);
    $s->bindValue(':mid', $_SESSION['testid']);
    $s->execute();
    $result = $s->fetchAll();
    $sql1 = "select * from shortquestionanswers where shostudid = :studid and shotestid = :testid limit $limit, $perpage";
    $s = $conn->prepare($sql1);
    $s->bindValue(':studid', $_SESSION['id']);
    $s->bindValue(':testid', $_SESSION['testid']);
    $s->execute();
    $answer = $s->fetchAll();
    $query = $conn->query("select count(*) from shortanswerques where shomid = '".$_SESSION['testid']."'");
    $total = $query->fetch();
...
<head>
    ...
    <title>PTE Preparation - Answer Short Question Answer Key</title>
...
<div class="container-fluid sidenav">
    <div class="row">
        ...
        <div class="col-md-8 mid">
            <h2>Question type: Answer Short Question</h2>
            <ul class="pagination">
                <?php for ($i=1; $i <= $pages; $i++) {?>
                <li><a href="?page=<?php echo $i?>">Item <?php echo $i?></a></li>
                <?php } ?>
                <li><a style="background-color: #082f6f; color: white;" href="anskeySummarizeText.html.php?page=1">Next Item Type</a></li>
            </ul>
            <?php foreach ($result as $row) {?>
            <div class="qbox">
                <h3>Item <?php echo $row['shoqno'];?></h3>
                <audio controls>
                    <source src="serveruploads/questions/answershortquestion/<?php echo $row['shofile']; ?>" type="audio/mpeg">
                </audio>
                <p><?php echo $row['shotext'];?></p>
                <?php foreach ($answer as $how) {?>
                <div class="youranswer">
                    <h3>Your Answer</h3>
                    <audio controls>
                        <source src="myphpuploaders/clientuploads/shortanswer/<?php echo $how['shofile']; ?>" type="audio/mpeg">
                    </audio>
                </div>
                <?php } ?>
                <div class="modanswer">
                    <h3>Model Answer</h3>
                    <p><?php echo $row['shoanskey'];?></p>
                </div>
            </div>
            <?php } ?>
